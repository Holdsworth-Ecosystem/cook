...
class TestDispatch:
    ...
    async def test_suggest_recipes_dispatches(self):
        ...
        with patch("cook.handlers.suggest.handle_suggest_recipes", mock_handler):
            await _dispatch("suggest_recipes", {"diners": ["Dave"]})
    ...
    async def test_get_dietary_profile_dispatches(self):
        ...
        with patch("cook.handlers.dietary.handle_get_dietary_profile", mock_handler):
            await _dispatch("get_dietary_profile", {"member_name": "Dave"})
    ...
    async def test_update_dietary_profile_dispatches(self):
        ...
        with patch("cook.handlers.dietary.handle_update_dietary_profile", mock_handler):
            await _dispatch(
    ...
    async def test_check_dietary_dispatches(self):
        ...
        with patch("cook.handlers.suggest.handle_check_dietary", mock_handler):
            await _dispatch("check_dietary", {"recipe_name": "Pasta"})
        mock_handler.assert_called_once()

    async def test_record_meal_dispatches(self):
        mock_handler = AsyncMock(return_value={"success": True})
        with patch("cook.handlers.meals.handle_record_meal", mock_handler):
            await _dispatch("record_meal", {"recipe_name": "Pasta", "diners": ["Dave"]})
        mock_handler.assert_called_once()

    async def test_rate_recipe_dispatches(self):
        mock_handler = AsyncMock(return_value={"success": True})
        with patch("cook.handlers.meals.handle_rate_recipe", mock_handler):
            await _dispatch(
    ...
    async def test_order_recipe_ingredients_dispatches(self):
        mock_handler = AsyncMock(return_value={"success": True})
        with patch("cook.handlers.order.handle_order_recipe_ingredients", mock_handler):
